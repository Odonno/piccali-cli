...
function SidebarProvider({
	...
	const setSidebarWidth = React.useCallback((width: number) => {
		const clamped = Math.max(
			...
			Math.min(SIDEBAR_WIDTH_PX_MAX, width),
		);
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	const { toggleSidebar, setSidebarWidth, sidebarWidth, open, setIsResizing } =
		useSidebar();
...
function SidebarMenuSub({ className, ...props }: React.ComponentProps<"ul">) {
	return (
		<ul
			...
			className={cn(
				"ml-3.5 mr-0 flex min-w-0 translate-x-px flex-col gap-1 border-l border-sidebar-border pl-2.5 pr-1 py-0.5 group-data-[collapsible=icon]:hidden",
				className,
			)}
			{...props}
		/>
